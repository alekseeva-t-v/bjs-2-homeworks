...
class Player {
  ...
  chooseEnemy(players) {
    ...
    const distanсes = minHealthPlayers.map((p) =>
      Math.abs(p.position - this.position)
    );
    const minDistance = Math.min(...distanсes);
    const enemy = players.find(
      (p) => Math.abs(p.position - this.position) === minDistance
    );
    return enemy;
  }
...
class Archer extends Player {
  ...
  getDamage(distance) {
    return (
      ((this.attack + this.weapon.attack) * this.getLuck() * distance) /
      this.weapon.range
    );
  }
}
